...
static shared_ptr<Evaluator> _parse(OptionParser &parser) {
    ...
    evaluator_type_doc.push_back(
        "XDP evaluator: Convex downward parabola priority function: f(n) = (2*w -1)h(n) + g(n) + sqrt((g(n)-h(n))^2 + 4*w*g(n)*h(n)");
    evaluator_type.push_back("XUP");
    evaluator_type_doc.push_back(
        "XUP evaluator: Convex upward parabola priority function: f(n) = h(n) + g(n) + sqrt((g(n)+h(n))^2 + 4*w*(w-1)*g(n)^2");
