...
SearchStatus EagerSearch::step() {
    ...
    while (true) {
        ...
        StateID id = open_list->remove_min(); //FOCAL LIST INVOLVEMENT?
...
void EagerSearch::reward_progress() { //CHANGE WITH IOS & FOCAL
...
create_ios_focal_list_factory_and_w_eval(const Options &opts) { //how to make easier - get weight here then apply and use pair?
    shared_ptr<GEval> g = make_shared<GEval>();
    shared_ptr<Evaluator> h = opts.get<shared_ptr<Evaluator>>("eval");
    shared_ptr<Evaluator> f = make_shared<SubEval>(vector<shared_ptr<Evaluator>>({g, h})); //added weight?
