#include "eager_search.h"
#include "search_common.h"

#include "../option_parser.h"
#include "../plugin.h"

using namespace std;

namespace plugin_eager {
static shared_ptr<SearchEngine> _parse(OptionParser &parser) {
    parser.document_synopsis("Improved Optimistic Search", "");

    parser.add_option<shared_ptr<OpenListFactory>>("open", "open list");
    parser.add_option<shared_ptr<OpenListFactory>>("focal", "focal list"); //focal list
    parser.add_option<bool>("reopen_closed",
                            "reopen closed nodes", "false");
    parser.add_option<shared_ptr<Evaluator>>(
        "f_eval",
        "set evaluator for jump statistics. "
        "(Optional; if no evaluator is used, jump statistics will not be displayed.)",
        OptionParser::NONE);
    parser.add_list_option<shared_ptr<Evaluator>>(
        "preferred",
        "use preferred operators of these evaluators", "[]");
        //Second evaluator for focal list

    eager_search::add_options_to_parser(parser);
    Options opts = parser.parse();

    shared_ptr<eager_search::EagerSearch> engine;
    if (!parser.dry_run()) {
    auto temp = search_common::create_astar_open_list_factory_and_f_eval(opts);
        opts.set("open", temp.first);
        opts.set("f_eval", temp.second);
        opts.set("reopen_closed", true);
        vector<shared_ptr<Evaluator>> preferred_list;
        opts.set("preferred", preferred_list);
        engine = make_shared<eager_search::EagerSearch>(opts);
        
    /*auto temp2 = search_common::create_ios_focal_list_factory_and_w_eval(opts); //change in search_common
        opts.set("focal", temp2.first);
        opts.set("w_eval", temp2.second);
        opts.set("reopen_closed", true);
        engine = make_shared<eager_search::EagerSearch>(opts); */
    }


    return engine;
}

static Plugin<SearchEngine> _plugin("ios", _parse);
}
